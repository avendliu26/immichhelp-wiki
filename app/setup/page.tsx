...
export default function Setup() {
  ...
  return <main className="container section category-page"><div className="eyebrow">Install &amp; Setup</div><h1>Setup</h1><p className="article-lead">Install, configure, verify, update, and roll back with confidence.</p>
    <section className="category-section" aria-labelledby="recommended-installation"><div className="section-heading"><h2 id="recommended-installation">Recommended Installation</h2><p>Start with the current, source-backed path.</p></div><div className="card-grid">{docs.filter((doc) => doc.slug === 'setup/immich-docker-compose').map((doc) => <div className="card" key={doc.slug}><div className="card-tag">{doc.category}</div><h3>Immich Docker Compose</h3><p>{doc.description}</p>{doc.lastReviewed && <small className="card-meta">Last reviewed {doc.lastReviewed}</small>}<Link className="primary-button" style={{marginTop:16}} href={`/${doc.slug}`}>Start Installation Guide →</Link></div>)}</div></section>
